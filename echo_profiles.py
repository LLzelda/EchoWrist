...
def check_integrity(raw_seq, pkt_length=240):
    ...
    bad_pkt = np.sum(np.abs(np.reshape(uniformed_seq, (-1, pkt_length))) < 1e-6, axis=1) == pkt_length
    n_bad_pkt = np.sum(bad_pkt)
    if n_bad_pkt:
        print('Bad packets: %d' % n_bad_pkt)
    bad_pkt = np.ones((uniformed_seq.shape[0] // pkt_length, pkt_length), dtype=bool) * bad_pkt[:, None]
    bad_samples = np.ones(raw_seq.shape, dtype=bool)
    bad_samples[:bad_pkt.ravel().shape[0]] = bad_pkt.ravel()
    return bad_samples

def echo_profiles(audio_file, audio_config, target_bitwidth, maxval, maxdiff, mindiff, rectify=False, no_overlapp=False, no_diff=False, coi_suffix = ''):

    original_audio, all_audio = load_audio(audio_file, audio_config['sample_depth'], target_bitwidth)
    ...
    if 'tx_file' in audio_config:
        ...
        raise RuntimeError('Unknown profiling method: %s' % audio_config['signal'])

    tx_signals = []
    for f in tx_files:
        _, this_tx = load_audio(os.path.join('tx_signals', f))
        frame_length = this_tx.shape[0]
        assert(frame_length == audio_config['frame_length'])    # frame_length must match
    ...
    for n, tx in enumerate(tx_signals):
        for i, c in enumerate(audio_config['channels_of_interest']):
            filtered_audio[n, i] = butter_bandpass_filter(all_audio[:, c], bp_ranges[n][0], bp_ranges[n][1], audio_config['sampling_rate'])
            # find the start pos
            if audio_config['signal'].lower() == 'gsm':
                channel_start_profiles = np.convolve(filtered_audio[n, i][:start_profiles.shape[2]], tx, mode='full')
            else:
            ...
            # print(n, i, start_profiles[n, i, :10])
    ...
    if no_overlapp:
            ...
            for c in range(n_coi):
                if audio_config['signal'].lower() == 'gsm':
                    for f in range(profiles.shape[1]):
                        profiles[n, c, f, :] = np.convolve(filtered_audio[n, c, f, :], tx, mode='full')[frame_length - 1:]
                else:
                    for f in range(profiles.shape[1]):
        ...
        for n, tx in enumerate(tx_signals):
            for c in range(n_coi):
                if audio_config['signal'].lower() == 'gsm':
                    profiles[n, c, :] = np.convolve(filtered_audio[n, c].ravel(), tx, mode='full')[frame_length - 1:]
                else:
    ...
    bad_samples = bad_samples[:len(bad_samples) - len(bad_samples) % frame_length]
    bad_samples = np.reshape(bad_samples[:, 0], (-1, frame_length)).T   # if one channel is bad, all channels are bad
    ...
    if not no_diff:
        ...
        # Normalization
        norm_diff = np.zeros(diff_profiles.shape)
        for col in range(diff_profiles.shape[1]):
            col_data = diff_profiles[:, col]
            diff_min = np.min(col_data)
            diff_max = np.max(col_data)
            if (diff_max - diff_min) != 0:  # Handle cases where the value is zero
                norm_diff[:, col] = (col_data - diff_min) / (diff_max - diff_min)
    ...
    # Normalization
    norm_echo = np.zeros(profiles.shape)
    for col in range(profiles.shape[1]):
        col_data = profiles[:, col]
        echo_min = np.min(col_data)
        echo_max = np.max(col_data)
        if (echo_max - echo_min) != 0:  # Handle cases where the value is zero
            norm_echo[:, col] = (col_data - echo_min) / (echo_max - echo_min)

    filename = ('%s_%s%s_%dbit_profiles' % (audio_file[:-4], audio_config['signal'].lower(), no_overlapp_text, target_bitwidth))
    cv2.imwrite(filename +'.png', profiles_img)
    np.save(filename + '.npy', profiles)   # TOCHANGE: Normalization profiles/norm_echo
    if not no_diff:
        filename = ('%s_%s%s_%dbit_diff_profiles' % (audio_file[:-4], audio_config['signal'].lower(), no_overlapp_text, target_bitwidth))
        cv2.imwrite(filename+'.png', diff_profiles_img)
        np.save(filename + '.npy' , diff_profiles)  # TOCHANGE: Normalization diff_profiles/norm_diff
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_diff', help='do not generate differential echo 1', action='store_true')
    ...
    parser.add_argument('--sample_depth', help='sampling depth (bit) of audio file, comma-separated', type=int, default=0)
    ...
    if len(args.channels_of_interest):
        audio_config['channels_of_interest'] = [int(x) for x in args.channels_of_interest.split(',')]
    ...
    if len(args.bandpass_range):
        audio_config['bandpass_range'] = [float(x) for x in args.bandpass_range.split(',')]
    ...
    echo_profiles(args.audio, audio_config, args.target_bitwidth, args.maxval, args.maxdiffval, args.mindiffval, args.rectify, args.no_overlapp, args.no_diff, coi_suffix)